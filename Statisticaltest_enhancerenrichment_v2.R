...
noOfGenesForSampling <- length(target_ranges)

distances <- as.integer( c( 10000, 1000000, 2000000))
for( i in 1:length( distances ))
{  
   #initiate empty vectors to hold values
   ranTotalGenesSigNumbers <- numeric()
   up_and_down_tss <- getUpstreamAndDownstreamCoordinatesOfTSS(as.data.frame( target_ranges),
   distances[i])
   #convert abouve data frame to GRanges object
   up_and_down_tss <- dataframe2GrangesObject(up_and_down_tss)
   ...
   ranTargetSigNumbers[i] <- noOfSignals_target 
   noOfSignals_RanCpG <- sampleGenesAndGiveQunatityOfSignals( cpg_ensembl_genes, 402, 402,
   distances[i], chip_Range )
   noOfSignals_RanCpG <- (noOfSignals_RanCpG/402) * noOfGenesForSampling
   ranCpGSigNumbers[i] <- noOfSignals_RanCpG
   noOfSignals_RanNonCpG <- sampleGenesAndGiveQunatityOfSignals( non_cpg_ensembl_genes, 1652, 1652,
   distances[i], chip_Range )
   noOfSignals_RanNonCpG <- (noOfSignals_RanNonCpG/1652) * noOfGenesForSampling
   ranNonCpGSigNumbers[i] <- noOfSignals_RanNonCpG
   noOfSignals_RannonTarget <- sampleGenesAndGiveQunatityOfSignals( nontarget_ranges, 23, 23,
   distances[i], chip_Range )
   noOfSignals_RannonTarget <- (noOfSignals_RannonTarget/23) * noOfGenesForSampling
   rannonTargetSigNumbers[i] <- noOfSignals_RannonTarget
   #Number of times to sample 

for ( x in 1:1000 )
   {
      noOfSignals_ranTotalgenes <- sampleGenesAndGiveQunatityOfSignals( ensembl_ranges, 2054, noOfGenesForSampling,
      distances[i], chip_Range )  
      ranTotalGenesSigNumbers[x] <- noOfSignals_ranTotalgenes  

   }
   #all_signals <- data.frame( ranTargetSigNumbers, ranCpGSigNumbers, ranNonCpGSigNumbers)
   #all_signals <- data.frame(ranTotalGenesSigNumbers, ranCpGSigNumbers, ranNonCpGSigNumbers)
   #output for 1000 samples
   all_signals <- data.frame(ranTotalGenesSigNumbers)
   #outfileName <- paste(slideNo,"all_chipSidnals", distances[i],"total", sep="_")
   outfileName1 <- paste(slideNo,"TotalGenes1000Samples", distances[i], sep="_")
   write.table( all_signals, file=outfileName1, col.names=T, row.names=F, quote=F, sep="\t")
   
}

   #output for single sample for each distance
   outFile2 <- paste(slideNo,"3Signals", sep="_") 
   write.table( data.frame( ranTargetSigNumbers,ranCpGSigNumbers,ranNonCpGSigNumbers,rannonTargetSigNumbers ), file=outFile2, 
